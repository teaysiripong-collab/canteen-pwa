...
**2. ตัวเลขมาจากเครื่องมือ ไม่ได้มาจากโมเดล** — โมเดลเลือกว่าจะเรียกเครื่องมือไหน เซิร์ฟเวอร์เป็นคนไปอ่านฐานข้อมูลจริง แล้วส่งแถวข้อมูลกลับไป ทุกตัวเลขในคำตอบจึงเป็นผลลัพธ์ของ query ที่คนก็รันเองได้ และคำตอบทุกคำตอบ **แนบลิงก์หน้าจอต้นทาง** ให้กดไปดูตัวเลขเดียวกันแบบเต็มๆ ได้
...
- ต้องมี `ANTHROPIC_API_KEY` ใน environment (**ห้าม commit ลง repo** — `.env.example` มีแค่ชื่อตัวแปร) ถ้าไม่มี หน้าผู้ช่วยจะบอกตรงๆ ว่ายังไม่ได้ตั้งค่า ส่วนอื่นของระบบทำงานปกติ
